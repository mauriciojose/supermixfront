...
class EditVariacoesProduto extends React.Component {
    constructor(props){
      super(props);

      let { produto_id } = this.props.match.params;
    ...
    componentDidMount(){
      ...
      Api.get(`/variacoes_produto/${this.state.produto_id}`,{}).then(res => {
          
        console.log(res.data);

        this.initVariations(res.data);
  ...
  carregaInformacoes(){
    ...
    submit(e){
      ...
      this.setState({loadingSend: true});
        ...
        Api.put(`/variacoes_produto/${this.state.produto_id}`, variacoes, {
        }).then(res => {
    ...
    existsInReferencia( variacoes, id ){
      for (const variacao of variacoes) {
        if ( variacao.value == id ) {
          return true;
        }
      }
      return false;
    ...
    setReferenciaVariacao( variacoes, refSelected ){
      for (const variacao of variacoes) {
        
        if ( refSelected[variacao.referencia] == undefined ) {
          refSelected[variacao.referencia] = [];
        }

        if ( !this.existsInReferencia( refSelected[variacao.referencia], variacao.variacao ) ) {
          
          refSelected[variacao.referencia].push({
    ...
    initVariations( variations ){
      ...
      for (let index = 0; index < variations.length; index++) {
        const variacaoNew = variations[index];

        let variacao = this.getKeyInit(variacaoNew.referencias);

        newsArray[variacao] = {
          id: variacaoNew.id,
          codigoOld: new Date().getTime(),
          codigo: variacaoNew.codigo,
          quantidade: parseInt(variacaoNew.quantidade),
          valor_venda: parseFloat( variacaoNew.valor_venda ) * 100,
          peso: parseFloat( variacaoNew.peso ) * 100,
          altura: parseFloat( variacaoNew.altura ) * 100, 
          largura: parseFloat( variacaoNew.largura ) * 100, 
          comprimento: parseFloat( variacaoNew.comprimento ) * 100,
          endereco: variacaoNew.endereco,
          ativo: variacaoNew.ativo,
          peso_id: parseInt(variacaoNew.peso_id),
          unidade_medida_id: parseInt(variacaoNew.unidade_medida_id),
          values: this.getTrVariacaoInit(variacaoNew.referencias),
          variacoes: this.getKeyArrayInit(variacaoNew.referencias)
        };

        refSelected = this.setReferenciaVariacao( variacaoNew.referencias, refSelected )
      ...
      this.setState({arrayVariationsNew: newsArray, refSelected: refSelected});
