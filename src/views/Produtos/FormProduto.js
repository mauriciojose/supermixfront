...
function FormProduto(props) {
    ...
    useEffect(() => {
        ...
        Api.get(`/subcategorias?key=ativo&value=true`,{}).then(res => {
  
            if (res.data?.length > 0) {
                let newSubcateorias = res.data.map((value, index, array) => {
                  return {
    ...
    const submit = (e) => {
        ...
        Api.post(`/produtos`, produto, {
            ...
            setTimeout(() => {
                let etapa = res.data.variacao ? 1 : 2;
                window.location=`/admin/produtos/create/${res.data.id}/${etapa}`; 
            }, 1000);
    ...
    const getStep = () => {
        ...
        switch (step) {
            case '0':
                ...
                break;
            case '1':
                stepComponent = <VariacoesProduto pro_id={pro_id}></VariacoesProduto>;
                break;
            case '2':
                stepComponent = <ProdutoFotos pro_id={pro_id}></ProdutoFotos>;
                break;
